...
\pdfinfo{
   /Author (Arun Mahadev, Dominik Krupke, S\'{a}ndor Fekete, and Aaron T. Becker)
...
\author{Arun Mahadev, Dominik Krupke, S\'{a}ndor Fekete, and Aaron T. Becker% <-this % stops a space
...
\begin{abstract}
Consider a swarm of particles in a 1D or 2D grid that can be tracked and controlled by an external agent, but control inputs are applied uniformly so that each particle experiences the same applied forces. 
This paper presents algorithms for (1) mapping: building a representation of the free and obstacle regions of the workspace;
...
\bibliography{IEEEabrv,bib/uniformMapping}
