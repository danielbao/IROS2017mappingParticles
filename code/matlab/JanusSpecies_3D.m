function [movecount,k,nodecount,init_config,fill] = JanusSpecies_3D(k,itr,max_steps,map,config_flag,fill_flag,fill,p1,p2,p3,p4,p5,p6,starting_config)
...
%% CF repeatedly moves particles to frontier cells until there are no more frontier cells left
    function CF()
        ...
        while(nnz(frontier_exp)>0&&G.movecount<max_steps)%While there are still unknowns, DFS begins
            ...
            temp5loc=G.ind5;
            temp6loc=G.ind6;
            moveSeq = BFS_Expansion_AVM_Janus(G.update_map,roboloc,temp1loc,temp2loc,temp3loc,temp4loc,temp5loc,temp6loc,frontier_vec); 
            ...
            for mvIn =1:steps%Move to the frontier on all particles
                moveto(moveSeq(mvIn));
                nodecount(iter)=length(find(explored_map==0|explored_map==11|explored_map==12|explored_map==13|explored_map==14|explored_map==15|explored_map==16|explored_map==6));%Update the nodes visited in each step
                iter=iter+1;
                makemymovie()
            end %end DFS
        end
...
%% nodes updates frontier_exp
    function nodes(robIndex)
        ...
        for mv_type=1:6%Check in 4 directions
            for c = 1:numel(robIndex)%Check for every robot
                ...
                k2 = G.zi(robIndex(c))+G.movetyp(mv_type,3);%Change z-index
                %If the cell has never been visited and isn't an obstacle
                if RobotVisits(i2,j2,k2) == 0 && mapped_obstacles(i2,j2,k2) == 0
                    frontier_exp(i2,j2,k2)=1;%It's a frontier!
                    if(G.mapnum==2 && (i2==1||i2==3))
                        frontier_exp(i2,j2,k2)==0;
                    end
                else
                    frontier_exp(i2,j2,k2)=0;%It isn't a frontier!
                end
            end
        end
    end
...
%% t_callback processes the joystick input
    function t_Callback(~,~)
        try
            ...
            Z=axis(joy, 3);     % Y-axis is joystick axis 2
...
%% Updating the map
    function updateMap()
        ...
        G.roboloc=find(current_map== 11 | current_map == 12 | current_map == 13 | current_map == 14 | current_map == 15 | current_map == 16);
        G.ind1=find(current_map==11);
        G.ind2=find(current_map==12);
        G.ind3=find(current_map==13);
        G.ind4=find(current_map==14);
        G.ind3=find(current_map==15);
        G.ind4=find(current_map==16);
        ...
        [G.type5x,G.type5y]=find(current_map== 15);
        [G.type6x,G.type6y]=find(current_map== 16);
...
%             G.i=scatter(G.Fronty,G.Frontx,'s','b','filled'); 
            ...
            G.type5plot=scatter(G.type5y,G.type5x,'o','c','filled'); 
            G.type6plot=scatter(G.type6y,G.type6x,'o','color 4','filled'); 
...
%             G.k=scatter(G.Unky,G.Unkx,'s','MarkerFaceColor',[0.5 0.5 0.5],'MarkerEdgeColor',[0.5 0.5 0.5]);
            ...
            set(G.type5plot,'SizeData',0.8*markerWidth^2);
            set(G.type6plot,'SizeData',0.8*markerWidth^2);
...
%             set(G.k,'SizeData',1.1*markerWidth^2);
        ...
        tempmap=current_map;
        tempmap(tempmap==11|tempmap==12|tempmap==13|tempmap==14 | tempmap==15 | tempmap==16)=2;
...
%             set(G.k,'XData',G.Unky,'Ydata',G.Unkx,'SizeData',1.1*markerWidth^2);
            ...
            set(G.type5plot,'XData',G.type5y,'Ydata',G.type5x,'SizeData',0.8*markerWidth^2);
            set(G.type6plot,'XData',G.type6y,'Ydata',G.type6x,'SizeData',0.8*markerWidth^2);
            uistack(G.type1plot,'top');
            uistack(G.type2plot,'top');
            uistack(G.type3plot,'top');
            uistack(G.type4plot,'top');
            uistack(G.type5plot,'top');
            uistack(G.type6plot,'top');
...
%% updateTitle updates title when called
    function updateTitle()
        ...
        title([num2str(G.movecount), ' moves, ',num2str(k),' particles, ', num2str(length(find(explored_map==0|explored_map==11|explored_map==12|explored_map==13|explored_map==14|explored_map==15|explored_map==15))), FC,', ', num2str(nnz(G.free)), ' free cells Itr ', num2str(itr)]);
...
%% SetupWorld setups map
    function [blk,free,robvec,Moves] = SetupWorld(mapnum)
        ...
        G.ci=ci;
        G.hi=hi;
        Moves = repmat( (1:numel(free))',1,6); %Repeat free four times across
