...
for i=100:50:100%size(temprun,1) %set the range of values for the function. For now it will run for no of robots=500,2000
    for j=1:size(janus1temprun,2)%set the numner of iterations for the function
        cell_data(j,1)=janus2temprun(i,j).nodecount(250);
        cell_data(j,2)=janus1temprun(i,j).nodecount(250);

    end
end
[p,tbl,stats] = anova1(cell_data);
[ht,pt] = ttest(cell_data(:,1), cell_data(:,2));
title('LeafMap exploration for Janusparticles at 250 moves');
