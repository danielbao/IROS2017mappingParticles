...
of the bicriteria search problem; this is the best possible.
\end{theorem}
...
As long as the freespace is connected, depth-first-search (DFS) is the optimal solution to mapping.
...
The $moveSeq$ is generated by a breadth-first-search (BFS) shortest path algorithm which requires the map $\mathbf{M}$, source $elect$, and the cells {\sc Frontier}$(\mathbf{M})$. A representative $moveSeq$ is $\langle u,r,d,d,r,u,\ldots\rangle$. 
...
Alg.~\ref{alg:ElectParticle} will explore the target frontier cell by the end of $moveSeq$. 
However, often with large-population swarms the whole $moveSeq$ need not be implemented. 
Every time {\sc Move\&\!Update} is called, the nearest frontier is updated and $moveSeq$ is also updated because as the particles start to move, the target frontier cell  might be explored by a non-elect  particle. 
...
\While{$|${\sc Frontier}$(\mathbf{M})| >0  $}
\State$moveSeq \gets$ BFS$(\mathbf{M},elect,${\sc Frontier}$(\mathbf{M}))$
...
In each loop of Alg.~\ref{alg:ClosestFrontier}, all the moves in $moveSeq$ are implemented to explore the target frontier cell since it is the shortest possible route to a frontier cell. 
...
BFS expands the search in a clockwise order, ${l,u,r,d}$, so the first move is $l$. 
...
\begin{algorithmic}[1]
\While{$|${\sc Frontier}$(\mathbf{M})| >0  $}
\State$moveSeq \gets$ BFS$(\mathbf{M},\mathbf{r},${\sc Frontier}$(\mathbf{M}))$
\For {$iter:=1$ $\mathbf{ to}$ $|moveSeq| $ $\mathbf{ step }$ $ 1$}  
...
However, in some scenarios the greedy strategy can perform worse than DFS with a single particle, e.g., in Fig.~\ref{fig:greed-worse-than-dfs}.
...
In this example, the greedy strategy using Alg.~\ref{alg:ClosestFrontier} is $1.5\times$ worse than DFS using only the bottom left particle.}
...
%TRIVIAL UPPER BOUND
\begin{theorem}
	The greedy strategy needs at most $0.5 \cdot m \cdot (m+1)$ moves where $m$ is the number of fields. 
\end{theorem}
\begin{proof}
...
If the move preference is counter clockwise, the greedy strategy Alg.~\ref{alg:ClosestFrontier} will go right first and then cover the square with a single particle which takes $\Omega(n^2)$ moves while the optimal strategy, which visit the square in parallel using all $n$ particles, only needs $O(n)$ moves.}
\end{figure}
...
%Comment: Just some quick text because this section is still missing. Feel free to remove or change.
\begin{theorem}
	The greedy strategy has a computational complexity in $O(m^2)$.
\end{theorem}
\begin{proof}
	For an environment with $m$ fields, there are at most $m$ iterations.
	Since the edges in the grid graph are not weighted and each field only has at most four neighbors, the shortest path from a particle to the boundary can be calculated in $O(m)$ time by a simple breadth first search.
