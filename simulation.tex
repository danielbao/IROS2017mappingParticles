...
\end{figure}
 ...
 The expected gap between the last particle and the boundary $\overline{p}$ is 90.98.
  ...
  In each run except Fig.~\ref{fig:RegionvsFloodvsUniform} the particles were placed randomly throughout the workspace. 
  
 %%%%%%%%%%%%%%%  MappingAlg3maps
  The comparison plot Fig.~\ref{fig:MappingAlg3maps} between the mapping of  three 2D mapsâ \emph{complex}, \emph{empty rectangle} and \emph{linear} and a 1D map, shows that the complex map requires the most moves. 
  In Fig.~\ref{fig:MappingAlg3maps} there is an observable difference in moves between the linear and rectangular workspaces. 
	...
	The number of cells to explore is $3m+2 = 15,002$ for the linear map, but only $m + 2(50+100) = 5300$ for the rectangular map. 
...
The complex map, empty rectangle map, and linear map (similar to a 1D map, but with obstacles to detect on top and bottom) each have 5000 free spaces.
}
\end{figure}
